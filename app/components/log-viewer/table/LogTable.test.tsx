...
describe("LogTable", () => {
  ...
  it("re-sorts when a sortable column header is clicked", async () => {
    ...
    let dataRows = screen.getAllByRole("row").filter((r) => r.getAttribute("tabindex") === "0");
    ...
    await user.click(screen.getByRole("columnheader", { name: "Service" }));
    dataRows = screen.getAllByRole("row").filter((r) => r.getAttribute("tabindex") === "0");
  ...
  it("collapses a row when it is clicked again", async () => {
    ...
    const row = () => screen.getAllByRole("row").filter((r) => r.getAttribute("tabindex") === "0")[0];
